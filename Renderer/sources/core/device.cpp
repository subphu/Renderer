...
namespace {
	...
	const bool CheckFeatureSupport( const VkPhysicalDevice device, VkPhysicalDeviceFeatures features, u32* supportedCount ) {
		...
		for (u32 i = 0; i < size; i++) {
			...
			supportedCount += pBoolSupported ? 1 : 0;
		}
		return supported;
	}

	const bool CheckDeviceExtensionSupport( const VkPhysicalDevice physicalDevice, const vector<const c8*>& extensions, u32* supportedCount ) {
		vkEnumerateDeviceExtensionProperties( physicalDevice, nullptr, supportedCount, nullptr );
		vector<VkExtensionProperties> availableExtensions( *supportedCount );
		vkEnumerateDeviceExtensionProperties( physicalDevice, nullptr, supportedCount, availableExtensions.data() );
...
u32 Device::isSupported( const VkSurfaceKHR surface ) {
	LOG("Device::setup");
	...
	u32 totalSupportedFeature = 0;
	u32 totalSupportedExtension = 0;
	...
	bool featureSupported	= CheckFeatureSupport( mPhysicalDevice, deviceFeatures, &totalSupportedFeature );
	bool extensionSupported	= CheckDeviceExtensionSupport( mPhysicalDevice, deviceExtensions, &totalSupportedExtension );
	...
	return mSupported ? U32(totalSupportedFeature + totalSupportedExtension + mSurfaceFormats.size() + mPresentModes.size()) : 0;
}
