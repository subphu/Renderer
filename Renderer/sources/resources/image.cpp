...
void Image::create( const UInt2 size, const VkFormat format ) {
	UInt3 size3 = { size.width, size.height, 1 };
	create( size3, format );
}
...
void Image::generateMipmaps( VkCommandBuffer cmdBuffer ) {
	...
	VkImageMemoryBarrier barrier = getMemoryBarrier();
	GenerateMipMap( cmdBuffer, barrier, mImageInfo.extent, mImageInfo.mipLevels );
}

VkImageMemoryBarrier Image::getMemoryBarrier() {
...
void Image::copyTo( const VkCommandBuffer cmdBuffer, const VkImage dstImage ) {

	VkImageCopy region{};
	region.extent = mImageInfo.extent;
	region.srcOffset = { 0, 0, 0 };
    region.srcSubresource.aspectMask     = mImageViewInfo.subresourceRange.aspectMask;
    region.srcSubresource.baseArrayLayer = mImageViewInfo.subresourceRange.baseArrayLayer;
    region.srcSubresource.layerCount     = mImageViewInfo.subresourceRange.layerCount;
    region.srcSubresource.mipLevel       = 0;

	region.dstOffset = { 0, 0, 0 };
    region.dstSubresource.aspectMask     = mImageViewInfo.subresourceRange.aspectMask;
    region.dstSubresource.baseArrayLayer = mImageViewInfo.subresourceRange.baseArrayLayer;
    region.dstSubresource.layerCount     = mImageViewInfo.subresourceRange.layerCount;
